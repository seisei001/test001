...
| url-viewer | 🔗 URLリンク内容ビューア | apps/url-viewer/ | URLを順に開いて内容を確認・編集・保存。Pages上は公開CORSプロキシ経由、ローカルは `npm start`(server.js)で `/api/fetch` を使用。「内容を保管」はWeb Share APIで共有シートを開き(非対応環境はダウンロード)、ファイル名はURL末尾番号→本文1行目「第N話」の順で `001.txt` 形式を自動入力(手修正可)。保存内容はエディタの本文のみ |
